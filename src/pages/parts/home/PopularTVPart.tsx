...
import { searchForPopularTvs } from "@/backend/metadata/search";
...
export function PopularTVPart({ searchQuery }: { searchQuery: string }) {
  const { t } = useTranslation();

  const [results, setResults] = useState<MediaItem[]>([]);
  const [state, exec] = useAsyncFn((query: MWQuery) =>
    searchForPopularTvs(query),
  );
    ...
    <div>
      {results.length > 0 ? (
        <div>
          ...
          <SectionHeading
            title={t("home.popularTVs.sectionTitle") || "Popular TV Shows"}
